...
if (isset($_SESSION['usuario'])){
    ...
    if (mysqli_num_rows($db_usuarios) == 0) {
        header("Location: /php/login/logout.php?error=usuario_inexistente&usuario='$usuario'"); //Redirige a logout
        die;
    }else{
        $usuario_completo = mysqli_fetch_assoc($db_usuarios);
    
        if ($usuario_completo['estado'] == 6){      //Verfificar si el usuario esta deshabilitado
            header("Location: /php/login/logout.php?error=usuario_deshabilitado&usuario='$usuario'"); //Redirige a logout
            die;
        }else{
